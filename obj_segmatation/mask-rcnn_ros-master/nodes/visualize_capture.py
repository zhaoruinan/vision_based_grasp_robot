...
def display_instances_cv(image, boxes, masks, class_ids, class_names,
    ...
    for i in range(n):        
        ...
        if not np.any(boxes[i]):
            ...
            continue
        y1, x1, y2, x2 = boxes[i]
        cv2.rectangle(tmp_img, (x1, y1), (x2, y2), color=bgr_color, thickness=1)

        # Draw transparent mask
        overlay = tmp_img.copy()
        mask = masks[:, :, i]
        crop_img = cv2.cvtColor(image,cv2.COLOR_BGR2BGRA)
        crop_img[:,:,3] = mask*255
        crop_img = crop_img[y1+1:y2-1,x1+1:x2-1]
        cropped_img.append(crop_img)
        # cv2.imwrite('/home/irobot/temp_mask/temp%i.png'%count,crop_img)
        # count += 1
        __, thresh = cv2.threshold(mask, 0.5, 1, cv2.THRESH_BINARY)
        _, contours, _ = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        cv2.drawContours(tmp_img, contours, -1, color=bgr_color, thickness=cv2.FILLED)
        cv2.addWeighted(overlay, alpha, tmp_img, 1 - alpha, 0, tmp_img)

        # Draw text label
        score = scores[i] if scores is not None else None
        label = class_names[class_id]
        caption = "{} {:.3f}".format(label, score) if score else label
        cv2.putText(tmp_img, caption, (x1, y1 + 12), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5,
                    color=(255, 255, 255))
    print(len(cropped_img))
    return tmp_img, cropped_img
    sleep(0.1)
